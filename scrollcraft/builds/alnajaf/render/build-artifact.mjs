...
// --preview takes the lighter encodes from preview-assets/ (render/encode-preview.sh)
const ASSET_DIR = process.argv.includes('--preview') ? 'preview-assets' : 'assets';
let out = body;
for (const f of fs.readdirSync(path.join(B, ASSET_DIR)).filter((f) => f.endsWith('.webp'))) out = out.split(`assets/${f}`).join(`data:image/webp;base64,${b64(`${ASSET_DIR}/${f}`)}`);
const clips = [];
out = out.replace(/data-src(-mobile)?="assets\/([^"]+\.mp4)"/g, (m, mob, file) => {
  if (!fs.existsSync(path.join(B, ASSET_DIR, file))) return m;
  let i = clips.findIndex((c) => c.file === file);
  if (i === -1) { i = clips.length; clips.push({ file, b64: b64(`${ASSET_DIR}/${file}`) }); }
...
console.log(`${dest}  ${mb.toFixed(1)} MB  (${clips.length} clip(s) embedded from ${ASSET_DIR}/)`);
